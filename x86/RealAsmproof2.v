...
Section WITHMEMORYMODEL.
  ...
  Inductive is_free : instruction -> Prop :=
    is_free_intro sz:
      is_free (Pfreeframe sz (Ptrofs.sub (Ptrofs.repr sz) (Ptrofs.repr (size_chunk Mptr)))).
  ...
  | match_states_free_ret
      ...
      (LOADRA: exists v, Mem.loadv Mptr m (rs2 RSP) = Some v)
      f
  ...
  Proof.
    ...
    - unfold rs0, rs1. intros. simpl_regs. rewrite (Pregmap.gso _ _ H0). auto.
  ...
  Lemma eval_addrmode_seq:
    forall (rs rs' : regset) (REQ: forall r, rs r = rs' r) a,
  ...
  Lemma eval_testcond_seq:
    forall (rs rs' : regset) (REQ: forall r, rs r = rs' r) t,
      eval_testcond t rs = eval_testcond t rs'.
  Proof.
    unfold eval_testcond. intros.
    repeat rewrite REQ. repeat destr.
  Qed.
  ...
  Proof.
    unfold exec_store; intros chunk m a r r' rd lrd sz r0 m0 SEQ EL; repeat destr_in EL.
    inv SEQ.
    erewrite eval_addrmode_seq in Heqo. rewrite <- REQ. rewrite Heqo. eauto. auto.
  Qed.
  ...
  Proof.
    ...
    - rewrite REQ.
      econstructor.
    - econstructor; eauto. rewrite <- REQ; eauto.
  Qed.
  ...
  Lemma extcall_progress:
    forall (rs1 rs2: regset)
      ...
      (RA_TYPE : Val.has_type (rs1 RA) Tptr)
      (SP_NOT_VUNDEF : rs1 RSP <> Vundef)
      (RA_NOT_VUNDEF : rs1 RA <> Vundef)
      m' t
  ...
  Proof.
    ...
    destruct (SAFE _  (star_refl _ _ _)) as [(r & FS)|(t & s2' & STEP)].
    {
      ...
      - inv MS; try congruence. inv SEQ.
        left. eexists; constructor; rewrite <- REQ; eauto.
    } 
    ...
    - exploit step_internal. apply STEP. eauto. intro IB; inv IB; inv ALLOC. intros (EI & T0).
      ...
      2: unfold Mem.storev in MEQ; destr_in MEQ; eauto.
    ...
    - inversion SEQ; subst.
      ...
      + inversion STEP; subst; rewrite_hyps.
          ...
          - assert (exists b f', Genv.find_funct_ptr ge b = Some f' /\
            ...
            + inv H.
              * exploit step_internal. apply STEP2. simpl. rewrite PC'. rewrite FFP.
                ...
                right; do 2 eexists. eapply exec_step_internal. rewrite <- REQ; eauto. eauto. eauto.
                simpl in H6. inv H6. simpl. force_rewrite_match Heqo. eauto.
                simpl_regs. rewrite <- ! REQ.
                f_equal.
                apply offset_ptr_neg_sub.
              * exploit step_internal. apply STEP2. simpl. rewrite PC'. rewrite FFP.
                erewrite wf_asm_alloc_at_beginning; eauto. inversion 1. intros (EI & T0). subst.
                simpl in EI. destr_in EI.
                right; do 2 eexists. eapply exec_step_internal. rewrite <- REQ; eauto. eauto. eauto.
            ...
            + generalize (step_external_store _ _ _ STEP2 e). simpl; rewrite PC' , FFP.
              ...
              * simpl in H6. inv H6. revert STORE. simpl_regs. intros.
                right; do 2 eexists. eapply exec_step_internal. rewrite <- REQ; eauto. eauto. eauto.
                simpl. rewrite <- ! REQ. rewrite STORE. eauto.
        ...
        repeat rewrite <- REQ. repeat destr_in H6. eauto.
        repeat rewrite <- REQ. repeat destr_in H6. eauto.
        repeat rewrite <- REQ. repeat destr_in H6. eauto.
        repeat rewrite <- REQ. repeat destr_in H6. eauto.
        repeat destr; eauto.
        eapply goto_label_seq; eauto.
        rewrite <- REQ; repeat destr_in H6; eauto.
        erewrite <- eval_testcond_seq by eauto. repeat destr_in H6; eauto using goto_label_seq.
        erewrite <- eval_testcond_seq by eauto. destr_in H6.
        erewrite <- eval_testcond_seq by eauto. repeat destr_in H6; eauto using goto_label_seq.
        rewrite <- REQ. destr. destr. eapply goto_label_seq. eauto. intros.
        apply set_reg_eq; auto.
        ...
        right; do 2 eexists. eapply exec_step_builtin. rewrite <- REQ; eauto. eauto. eauto. eauto. eauto. auto. eauto. auto.
  ...
  Proof.
    ...
    - simpl in PC1. repeat destr_in PC1.
      ...
      rewrite RRSP.
      eapply wf_asm_wf_allocframe in Heqo0. inv Heqo0.
      unfold Mem.storev in MEQ; repeat destr_in MEQ. simpl. f_equal.
      apply ptrofs_cancel. eauto.
  ...
  Proof.
    ...
    - simpl; intros s1 IS1. inv IS1. inv H0.
      ...
      eexists; econstructor. eauto. econstructor; eauto. simpl. rewrite Ptrofs.unsigned_repr. eauto.
    ...
    - simpl; intros s1 s2 r MS FS; inv FS.
      ...
      inv SEQ. simpl in *.
      constructor. rewrite REQ. auto. rewrite REQ; auto.
    - simpl.
