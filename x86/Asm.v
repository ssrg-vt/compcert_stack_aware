...
Definition exec_instr {exec_load exec_store} `{!MemAccessors exec_load exec_store} {F V} (ge: Genv.t F V) (f: function) (i: instr_with_info) (rs: regset) (m: mem): outcome :=
  ...
  | Pcall_s id sg =>
      Next (rs#RA <- (Val.offset_ptr rs#PC sz) #PC <- (Genv.symbol_address ge id Ptrofs.zero)) m
  | Pcall_r r sg =>
  ...
  | Pallocframe fi ofs_ra ofs_link =>
    if check_alloc_frame fi ofs_link ofs_ra then
      match Mem.push_frame m fi ((Mptr, ofs_link, rs#RSP)::(Mptr,ofs_ra, rs#RA)::nil) with
        Some (m4,b) =>
        Next (nextinstr (rs #RAX <- (rs#RSP) #RSP <- (Vptr b Ptrofs.zero)) sz) m4
      | _ => Stuck
      end
    else Stuck
                ...
                then
                  ...
                  | Some m' =>
                    match Mem.unrecord_stack_block m' with
                      Some m' => Next (nextinstr (rs#RSP <- sp #RA <- ra) sz) m'
                    | None => Stuck
                    end
                  end
                else Stuck
