...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Lcall:
      ...
      step (State s f sp (Lcall sig ros :: b) rs m)
        E0 (Callstate (Stackframe f sp rs b:: s) f' rs m (fn_stack_requirements id) false)
  | exec_Ltailcall:
      ...
      step (State s f (Vptr stk Ptrofs.zero) (Ltailcall sig ros :: b) rs m)
        E0 (Callstate s f' rs' m' (fn_stack_requirements id) true)
  | exec_Lbuiltin:
  ...
  | exec_function_internal:
      forall s f rs m rs' m' stk m'' sz (tail:bool),
        Mem.alloc m 0 f.(fn_stacksize) = (m', stk) ->
        Mem.record_stack_blocks (if tail then m' else Mem.push_new_stage m') (make_singleton_frame_adt stk (fn_stacksize f) sz) = Some m'' ->
      rs' = undef_regs destroyed_at_function_entry (call_regs rs) ->
      step (Callstate s (Internal f) rs m sz tail)
        E0 (State s f (Vptr stk Ptrofs.zero) f.(fn_code) rs' m'')
  | exec_function_external:
      forall s ef args res rs1 rs2 m t m' sz tc,
      ...
      step (Callstate s (External ef) rs1 m sz tc)
