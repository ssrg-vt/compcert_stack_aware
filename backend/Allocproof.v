...
Lemma exec_moves:
  ...
  wt_regset env rs ->
  exists ls',
    star (step fn_stack_requirements init_ls) tge (Block s f sp (expand_moves mv bb) ls m)
  ...
  | match_stackframes_cons:
      forall res f sp pc rs s tf bb ls ts sg an e env
        ...
        (STEPS: forall v ls1 m,
           ...
           exists ls2,
           star (LTL.step fn_stack_requirements init_ls) tge (Block ts tf sp bb ls1 m)
...
- set (sg := RTL.funsig fd) in *.
  ...
  inv SI. inv MSA1. eapply Mem.free_top_tframe_no_perm; eauto.
  destruct (transf_function_inv _ _ FUN). congruence.
  intros (m2' & TC & EXT).
...

  {
    ...
    - destruct RIF as (b & o & EQ0 & EQ1).
      ...
      destruct is_callee_save eqn:?; eauto. discriminate.
    - congruence.
    - congruence.
    - destruct ident_eq; try congruence.
  }
  ...
  repeat rewrite_stack_blocks; eauto.
  intros A B; rewrite A, B in STRUCT. inv STRUCT; constructor; auto.
  red. simpl. split; auto. inv LF2. red in H4. red.
  red in H1. repeat destr_in H1. simpl. constructor; auto.
...
- monadInv FUN. simpl in *.
  ...
  intros [tm' [U V]].
  exploit Mem.record_push_extends_flat_alloc. apply H. rewrite H9. eauto. all: eauto.
  inv SI. auto.
  ...
  intros AA BB; rewrite AA, BB in STRUCT.
  inv STRUCT; constructor; eauto. repeat constructor; auto.
  red. simpl. inv LF2; auto.
...
Lemma initial_states_simulation:
  ...
  exists st2, LTL.initial_state fn_stack_requirements tprog st2 /\ match_states (Locmap.init Vundef) (Some Tint) st1 st2.
Proof.
  intros. inv H.
