...
| rsb_intro:
    forall (VF:valid_frame f m)
      (NOTIN: Forall (fun x => ~ in_stack (stack_adt m) x) (map fst (frame_adt_blocks f)))
      (RNG: Forall (fun b => forall o k p, Mem.perm m (fst b) o k p ->
                                   0 <= o < frame_size (snd b))%Z (frame_adt_blocks f))
      (SZ: (size_stack (stack_adt m) + size_frames (f,nil) < stack_limit)%Z),
...
Qed.
Next Obligation.
  red in H3. unfold get_frames_blocks in H3. simpl in H3.
  rewrite app_nil_r in H3. unfold get_frame_blocks in H3. simpl in H3.
...
Qed.
Next Obligation.
  unfold size_frames. simpl. omega.
Qed.
Next Obligation.
  eapply mem_bounds_perm; eauto.
Qed.
...
Next Obligation.
  constructor; auto.
  intros. eapply do_stores_perm_inv in H0. 2: eauto. red in H0. simpl in H0.
...
Next Obligation.
  ...
  injection Heq_anonymous0. intros -> ->. simpl in *. rewrite Heq_anonymous. omega.
Qed.
...
Inductive push_frame_spec: mem -> frame_info -> list (memory_chunk * ptrofs * val) -> mem -> block -> Prop :=
  pfs_intro:
    forall m fi l malloc b mstores mpf,
      ...
      record_stack_blocks mstores {| frame_adt_blocks := (b,fi)::nil;
...
Proof.
  ...
  eapply (destr_dep_let (alloc m 0 (frame_size fi)) (m', b) ). apply H. clear H.
  intros m0 b0 pf x A.
  ...
  + simpl.
    ...
    intros. eapply do_stores_perm_inv in H; eauto.
    unfold alloc in pf. inv pf. red in H; simpl in H.
    rewrite PMap.gss in H. destr_in H. 2: inv H.
    apply zle_zlt in Heqb0. auto.
  + simpl. constructor; auto.
    erewrite do_stores_stack_adt. 2: eauto.
    erewrite alloc_stack_adt; eauto.
    intro INF; eapply stack_valid in INF.
...
Proof.
  ...
  rewrite <- (proof_irr (norepet_1 b) frame_adt_blocks_norepet).
  rewrite <- (proof_irr (Z.le_max_l _ _) frame_adt_size_pos); auto.
Qed.
...
Proof.
  ...
  + unfold flat_frameinj. constructor.
    ...
    * intros i1 i2 f1 f2 FAP1 FAP2 A LT; repeat destr_in A.
...
Proof.
  ...
  - (* Some a -> Some a*)
    ...
    + generalize (stack_perms m1); rewrite Forall_forall; intro A.
      edestruct get_assoc_spec as (fr & IN1 & IN2). setoid_rewrite <- H. eauto.
      specialize (A _ IN2).
      red in A. simpl in *.
      specialize (fun o => A _ _ o Cur p IN1).
...
Proof.
  ...
  unfold flat_frameinj. rewrite EQ; simpl. auto.
  intros (m2' & A & B).
  eexists; split; eauto.
...
Proof.
  ...
  - intros.
    repeat destr_in G1. 
    + exists f2; split. constructor; simpl; auto. inv FAP1. simpl in H; inv H. auto.
  ...
  - intros.
    ...
    destruct INS2.
    + subst.
       exfalso; apply NIN1. left.
       rewrite INF; eauto.
       eapply in_frame_blocks_in_frame; eauto. 
    + eapply stack_inject_not_in_frames; eauto. simpl in NIN1; intuition.
  ...
  - intros. destr_in G.
    + inv G. inv FAP1; inv FAP2. simpl in *. inv H; inv H0. omega.
...
Proof.
  ...
  - intros. eapply stack_inject_not_in_frames; eauto. simpl in NIN1; intuition.
...
Proof.
  ...
  - intros.
    rewrite Forall_forall in BNDS.
    specialize (BNDS _ H). eauto.
...
Proof.
  ...
  erewrite in_lnr_get_assoc; eauto. eexists; split; eauto.
...
Proof.
  ...
  - generalize (record_stack_blocks_bounds _ _ _ H0).
...
Lemma record_stack_inject_left_zero:
  forall j g m1 s1 s2 f1 f2
    ...
    (FI: frame_inject j f1 f2)
    (SI: stack_inject j g m1 s1 s2)
    (SZ1: Forall (fun f => 0 <= frame_adt_size f)%Z s1)
    (SZ2: Forall (fun f => 0 = frame_adt_size f)%Z s2)
    (SZf1: (0 <= frame_adt_size f1)%Z)
  ,
    stack_inject j (fun n : nat => if Nat.eq_dec n 0 then Some 0 else g (Init.Nat.pred n)) m1 (f1 :: s1) s2.
Proof.
  intros j g m1 s1 s2 f1 f2 FAP FI SI SZ1 SZ2 SZf1.
  destruct SI.
  constructor.
  - red; intros. destr_in G1. inv G1. omega.
    destr_in G2. omega. 
  ...
  - intros. destr_in G; inv G. omega.
    apply stack_inject_pack in H0. omega.
  - intros. destr_in G.
    + subst. inv G. inv FAP1. simpl in H; inv H.
      apply frame_at_pos_In in FAP2.
      rewrite Forall_forall in SZ2. apply SZ2 in FAP2. rewrite <- FAP2. omega.        
    + apply frame_at_pos_cons_inv in FAP1; try omega.
      apply frame_at_pos_In in FAP1.
      apply frame_at_pos_In in FAP2.
      rewrite Forall_forall in SZ1, SZ2.
      apply SZ2 in FAP2.
      apply SZ1 in FAP1. omega. 
  - red; intros i LT.
...
Lemma record_stack_blocks_mem_inj_left_zero:
  forall j g m1 m2 f1 f2 m1',
    ...
    frame_at_pos (stack_adt m2) O f2 ->
    frame_inject j f1 f2 ->
    Forall (fun f => 0 <= frame_adt_size f)%Z (stack_adt m1) ->
    Forall (fun f => 0 = frame_adt_size f)%Z (stack_adt m2) ->
    (0 <= frame_adt_size f1)%Z ->
    mem_inj j (fun n => if Nat.eq_dec n 0 then Some 0 else g (pred n)) m1' m2.
Proof.
  intros j g m1 m2 f1 f2 m1' INJ ADT FAP FI SZ1 SZ2 SZf1; autospe.
  inv ADT.
  inversion INJ; subst; constructor; simpl; intros; eauto.
  eapply stack_inject_invariant_strong.
  intros. change (perm m1 b ofs k p) in H0. apply H0.
...
Lemma unrecord_stack_inject_left_zero:
  forall j g m1 f s1 s2
    ...
    stack_inject j (fun n : nat => g (S n)) m1 s1 s2.
Proof.
  intros j g m1 f s1 s2 SI All0 Ginit TOPNOPERM.
  destruct SI; constructor; auto.
  + red; intros.
    simpl in *. eapply stack_inject_preserves_order. 2: apply G1. 2: apply G2. omega.
  + intros i1 f1 FAP PERM.
...
Qed.
  ...
  Lemma record_stack_blocks_intro:
    forall m1 f,
      valid_frame f m1 ->
      Forall (fun b => ~ in_frames (stack_adt m1) (b)) (map fst (frame_adt_blocks f)) ->
      Forall
        (fun b  =>
           forall (o : Z) (k : perm_kind) (p : permission),
             perm m1 (fst b) o k p ->  (0 <= o < frame_size (snd b))%Z) 
        (frame_adt_blocks f) ->
  ...
  Proof.
    ...
    - red. intros i1 i2 LE i1' i2' G1 G2.
      ...
      specialize (stack_inject_preserves_order _ _ LE _ _ Heqo Heqo0). omega.
    - intros i1 f1 FAP PERM.
      destr; eauto.
      + subst. exfalso.
        inv FAP.
    ...
    - intros b1 b2 delta f2 fi FB NIN1 INS2 INF2 o k p PERM IPC.
      eapply stack_inject_not_in_frames; eauto. right; auto.
    - intros i j0 G.
    ...
    - intros i1 i2 f1 f2 FAP1 FAP2 G LARGEST.
      unfold option_map in G.
      repeat destr_in G.
      eapply stack_inject_sizes; eauto.
      eapply frame_at_pos_cons in FAP2.
      erewrite <- S_pred in FAP2. eauto.
      eapply NO0; eauto. omega.
      intros.
      eapply LARGEST.
      rewrite H. simpl. destr. subst.
      exfalso.
      rewrite Ginit in H. inv H.
      eapply NO0 in Heqo; omega.
    - red; intros.
  ...
  Lemma inject_tailcall_inlined:
    forall j j' g P1 P2 f f' s1 s2
      (SI: stack_inject j g P1 (f::s1) s2)
      (INCR: inject_incr j j')
      (G0: g O = Some O)
      (JB: forall b, in_frames (f::s1) b -> exists b' delta, j b = Some (b', delta))
      (* (JB: forall b, in_frame f b -> exists b' delta, j b = Some (b', delta)) *)
      (PERMS: forall b, in_frames s1 b -> forall o k p, P1 b o k p <-> P2 b o k p)
      (PERMS': forall b, ~ in_frame f' b -> forall o k p, P2 b o k p -> P1 b o k p)
      f2 (FAPtarget: frame_at_pos s2 O f2)
      (FI: frame_inject j' f' f2)
      (NEW: forall b1 b2 delta, j b1 = None -> j' b1 = Some (b2, delta) ->
                           ~ in_frames s1 b1)
      (NEW': forall b1 b2 delta, j b1 = None -> j' b1 = Some (b2, delta) ->
                            in_frame f' b1)
      (DIFF: forall b, in_frame f' b -> in_frame f b -> False)
      (NEWF: forall b, in_frame f' b -> j b = None)
      (OLDF: forall b, in_frame f b -> ~ exists o k p, P2 b o k p)
      (SIZES: (frame_adt_size f2 <= frame_adt_size f')%Z)
    ,
      stack_inject j' g P2 (f'::s1) s2.
  Proof.
    intros.
    inv SI.
    ...
    - destruct i1. eauto.
      + apply frame_at_pos_cons_inv in FAP. 2: omega.
        ...
        right. eapply in_frames_in_frame; eauto.
    ...
    - destruct (j b1) eqn: JB1.
      + destruct p0.
        ...
        eapply stack_inject_not_in_frames; eauto. simpl.
        intros [A|A].
        eapply OLDF; eauto.
        intuition.
      + specialize (NEW' _ _ _ JB1 FB). intuition.
    - destruct i1.
      + rewrite G0 in G; inv G.
        inv FAP1. simpl in H; inv H.
        exploit frame_at_same_pos. apply FAPtarget.  apply FAP2. intro; subst.
        auto.
      + apply frame_at_pos_cons_inv in FAP1. 2: omega.
        simpl in FAP1.
        eapply stack_inject_sizes; eauto.
...
,
  ...
  Proof.
    ...
    - intros b1 b2 delta1 ofs k p FB PERM IPC.
      ...
      eapply perm_alloc_inv in PERM; eauto.
      unfold F' in FB. repeat destr_in FB.
      + rewrite pred_dec_true in PERM; auto.
        eapply Mem.perm_cur. eapply perm_implies. eapply PERMinjstk0. auto. constructor.
      + rewrite pred_dec_false in PERM; auto.
        eapply mi_perm0; eauto.
        rewrite PERMUSB in PERM.
        eapply perm_free_3; eauto.
    - unfold F'.
      ...
      + apply DIV.
        ...
        eapply perm_alloc_3 in RP1; eauto.
        specialize (RP ofs). trim RP. generalize (size_chunk_pos chunk); omega.
        rewrite PERMRSB in RP.
        eapply perm_alloc_3 in RP; eauto.
        omega.
      + eapply mi_align0; eauto.
        red; intros.
        specialize (RP _ H).
        rewrite PERMRSB in RP.
        eapply perm_alloc_4 in RP. 2:  eauto. 2: auto.
        rewrite PERMUSB in RP.
        eapply perm_free_3; eauto.
    - unfold F'; intros b1 ofs b2 delta1 FB PERM.
      eapply memval_inject_incr.
      Focus 2.
      instantiate (1:=F). intros b1' b2' delta'. destr. subst.
      intro FS. apply VALID in FS.
      contradict FS.
      unfold valid_block.
      erewrite <- nextblock_free. 2: now eauto.
      erewrite <- NB'.
      eapply fresh_block_alloc; eauto.
      cut (b1 <> stk0 -> memval_inject F (ZMap.get ofs (mem_contents m) # b1) (ZMap.get (ofs + delta1)%Z (mem_contents m'0) # b2)).
      {
        inv RSB. simpl in *.
        unfold alloc in ALLOC; inv ALLOC. simpl in *.
        unfold_unrecord. simpl in *.
        unfold free in FREE; repeat destr_in FREE. simpl in *.
        clear.
        intro.
        rewrite PMap.gsspec. destr. rewrite ZMap.gi. constructor.
      }
      intro DIFF.
      rewrite pred_dec_false in FB; auto.
      rewrite PERMRSB in PERM.
      eapply perm_alloc_4 in PERM. 2:  eauto. 2: auto.
      rewrite PERMUSB in PERM.
      eapply perm_free_3 in PERM; eauto.         
    - rewrite <- (free_stack_adt  _ _ _ _ _ FREE) in mi_stack_blocks0.
      destruct (unrecord_stack_adt _ _ USB) as (x & EQ). rewrite EQ in mi_stack_blocks0.
      rewrite (record_stack_blocks_stack_adt _ _ _ RSB).
      rewrite (alloc_stack_adt _ _ _ _ _ ALLOC).
      eapply inject_tailcall_inlined. apply mi_stack_blocks0.
      + intros b1' b2' delta'. unfold F'; destr. subst.
        intro FS. apply VALID in FS.
        contradict FS. 
        unfold valid_block.
        erewrite <- nextblock_free. 2: now eauto.
      ...
      + rewrite <- EQ.
        erewrite free_stack_adt; eauto.
      + intros b IFR.
        assert (valid_block m'' b). eapply in_frames_valid; eauto.
        assert (b <> stk0). intro; subst. eapply fresh_block_alloc; eauto.
        assert (b <> stk). generalize (stack_norepet m'). rewrite EQ. intro ND; inv ND. intro; subst.
        eapply H4 in IFR; eauto.
        erewrite <- free_stack_adt in STACKTOP; eauto.
        rewrite EQ in STACKTOP. simpl in STACKTOP.
        red; rewrite STACKTOP. left; auto.
        intros o k p.
        rewrite PERMRSB.
        split; intro PERM.
        eapply perm_alloc_1; eauto. eapply PERMUSB. 2: eauto. eapply perm_free_1; eauto.
        eapply perm_free_3. eauto. eapply PERMUSB. eapply perm_alloc_4. eauto. auto. auto.
      + intros b. unfold in_frame; simpl.
        ...
        rewrite PERMUSB in PERM.
        eapply perm_free_3 in PERM; eauto.
      + destruct STACKTOP' as (r & STACKTOP').
      ...
      + unfold F'. intros b1 b2 delta1 N S.
        destr_in S. inv S. intro IS. apply in_frames_valid in IS.
        eapply fresh_block_alloc in IS; eauto.
      + unfold F'. intros b1 b2 delta1 N S.
        destr_in S. inv S. left; auto. 
      + intros b [|[]]. subst. simpl in *. intro IFR.
        exploit in_frames_valid. rewrite EQ. left; eauto.
        unfold valid_block. rewrite <- NB'.
        eapply fresh_block_alloc; eauto.
      + intros b [|[]]. subst. simpl in *.
        destruct (F stk0) eqn: AA; auto. destruct p. apply VALID in AA.
        eapply valid_block_free_1 in AA. 2: now eauto.
        unfold valid_block in AA. rewrite <- NB' in AA.
        eapply fresh_block_alloc in AA; eauto. easy.
      + erewrite <- free_stack_adt in STACKTOP. 2: now eauto.
        rewrite EQ in STACKTOP. simpl in STACKTOP.
        unfold in_frame. rewrite STACKTOP. intros b [|[]]; subst.
        intros (o & k & p & PERM).
        rewrite PERMRSB in PERM.
        eapply perm_alloc_inv in PERM. 2:  eauto.
        rewrite pred_dec_false in PERM.
        rewrite PERMUSB in PERM.
        exploit PERMstk; eauto.
        eapply perm_free_3 in PERM; eauto.
        intro RNG_stk.
        eapply perm_free_2 in PERM; eauto.
        intro; subst. clear PERM.
        exploit in_frames_valid.
        rewrite EQ. left; auto. red; rewrite STACKTOP. left; auto.
        intro AA. unfold valid_block in AA. rewrite <- NB' in AA.
        eapply fresh_block_alloc in AA; eauto.
      + simpl. auto.
  Qed.
  ...
  Proof.
    ...
    - simpl. intro MINJ.
      ...
      + unfold F'.
        ...
        red. rewrite NB'. eapply valid_block_free_1; eauto.
      + unfold F' in H.
        repeat destr_in H.
        eapply in_frames_valid. destruct STACKTOP' as (r & STACKTOP'); rewrite STACKTOP'. left; left; auto.
        eapply mi_mappedblocks0; eauto.
